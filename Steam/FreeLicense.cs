    ...
    class FreeLicense : ClientMsgHandler
    {
        ...
        private static void HandleClientRequestFreeLicenseResponse(IPacketMsg packetMsg)
        {
            ...
            if (packageIDs.Count > 0)
            {
                ...
                {
                    IRC.Instance.SendMain("{0}{1}{2} new free licenses granted", Colors.BLUE, packageIDs.Count, Colors.NORMAL);
                }
                else
                {
                    foreach (var package in packageIDs)
                    {
                        IRC.Instance.SendMain("New free license granted: {0}{1}{2} -{3} {4}", Colors.BLUE, Steam.GetPackageName(package), Colors.NORMAL, Colors.DARKBLUE, SteamDB.GetPackageURL(package));
                    }
                }
            }
